...
public class PointProductServiceImpl implements PointProductService {
    ...
    public List<PointProductDO> getAllEffectiveProducts(Date curDate, String productVersion, Long uid, String publishPlatFormType) {
        ...
        if (pointProductDOS != null) {
            pointProductDOS = pointProductDOS.stream().filter(h -> h.getExchangeStarttime().before(curDate) && h.getExchangeEndtime().after(curDate) && h.getProductVersion().contains(productVersion) && h.getPublishPlatFormType().contains(publishPlatFormType)).collect(Collectors.toList());
        }

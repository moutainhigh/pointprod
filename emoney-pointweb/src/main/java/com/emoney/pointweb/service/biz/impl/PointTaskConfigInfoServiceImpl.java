...
public class PointTaskConfigInfoServiceImpl implements PointTaskConfigInfoService {
    ...
    public List<PointTaskConfigInfoDO> getAllEffectiveTasks(Date curDate, Long uid, String productVersion, String publishPlatFormType) {
        ...
        if (pointTaskConfigInfoDOS != null) {
            pointTaskConfigInfoDOS = pointTaskConfigInfoDOS.stream().filter(h -> h.getTaskStartTime().before(curDate) && h.getTaskEndTime().after(curDate) && h.getProductVersion().contains(productVersion) && h.getPublishPlatFormType().contains(publishPlatFormType)).collect(Collectors.toList());
        }
        //接入用户画像
        if (pointTaskConfigInfoDOS != null) {

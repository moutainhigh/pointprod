...
var dialog = renderer["a" /* default */].create('<div class="modal note-modal" aria-hidden="false" tabindex="-1" role="dialog"/>', function ($node, options) {
  ...
  $node.html(['<div class="modal-dialog sn_close">', '<div class="modal-content">', options.title ? '<div class="modal-header">' + '<button type="button" class="close"  aria-label="Close" aria-hidden="true" onclick="CloseModal()">&times;</button>' + '<h4 class="modal-title">' + options.title + '</h4>' + '</div>' : '', '<div class="modal-body">' + options.body + '</div>', options.footer ? '<div class="modal-footer">' + options.footer + '</div>' : '', '</div>', '</div>'].join(''));
});
...
var ui_ui = function ui(editorOptions) {
  return {
    ...
    showDialog: function showDialog($dialog) {
      //var msh=$('.modal')[0].scrollTop;
      $dialog.modal('show');
      //if($('.fullscreen').length<=0){
            //var sch=window.screen.availHeight;
           // msh=Math.max(sch/2,msh);
            //$('.sn_close').css('top',msh+'px');
            //$('.modal')[0].scrollTop=msh;
      //}
    },
